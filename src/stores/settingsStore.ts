...
import { ApplicationService, type LscssSettings } from "../services";

interface SettingsState {
  settings: LscssSettings | null;
  isLoading: boolean;
  error: string | null;
  successMessage: string | null;

  loadSettings: () => Promise<void>;
  updateSettings: (updates: Partial<LscssSettings>) => Promise<void>;
...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  updateSettings: async (updates: Partial<LscssSettings>) => {
  ...
  exportData: async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `lscs-backup-${new Date().toISOString().split("T")[0]}.json`;
      a.click();
